...
func TestSaramaKafkaConsumerGroupHandler_SuccessProceedMessage_WithoutEventHandler_WithoutDLQHandler(t *testing.T) {
	...
	cgh := pubsub.NewDefaultSaramaConsumerGroupHandler(apm.DefaultTracer, "service-test", nil, nil)
	cgh.Setup(cgSess)
...
func TestSaramaKafkaConsumerGroupHandler_SuccessProceedMessage_WithEventHandler_WithoutDLQHandler(t *testing.T) {
	...
	cgh := pubsub.NewDefaultSaramaConsumerGroupHandler(apm.DefaultTracer, "service-test", eventHandler, nil)
	cgh.Setup(cgSess)
	cgh.ConsumeClaim(cgSess, cgClaim)
	cgh.Cleanup(cgSess)
...
func TestSaramaKafkaConsumerGroupHandler_ErrorProceedMessage_WithEventHandler_WithDLQHandler(t *testing.T) {
	...
	cgh := pubsub.NewDefaultSaramaConsumerGroupHandler(apm.DefaultTracer, "service-test", eventHandler, dlqHandler)
	cgh.Setup(cgSess)
	cgh.ConsumeClaim(cgSess, cgClaim)
	cgh.Cleanup(cgSess)
